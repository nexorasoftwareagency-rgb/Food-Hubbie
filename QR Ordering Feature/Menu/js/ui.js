...
export function updateGreeting(name) {
    ...
    if (!el) return;
    if (name && name.trim()) {
        el.textContent = `Hello, ${name.trim()} 👋`;
...
export function updateCheckoutFields(session) {
    ...
    if (name) {
        ...
        if (knownLabel) knownLabel.textContent = `Ordering as ${name}${phone ? ` · ${phone}` : ''}`;
...
export function statusChangeNotification(status, orderId) {
    const messages = {
        Placed: 'Order received! 🎉',
        Confirmed: 'Order confirmed! Being prepared 👨‍🍳',
        Preparing: 'Your meal is being prepared 🔥',
        Ready: 'Order ready to serve! 🍽️',
        Delivered: 'Order served! Enjoy your meal ❤️',
        Cancelled: 'Order cancelled ❌'
    };
    showToast(messages[status] || `Status updated: ${status}`);
}
...
export function renderTracking(orderId, order, tableNumber) {
    document.getElementById('trackingOrderId').textContent = `#FH-T${String(tableNumber).padStart(2, '0')}-${String(orderId).slice(-3).toUpperCase()}`;
