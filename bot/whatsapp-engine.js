/**
 * ============================================================
 * FOODHUBBIE SAAS — WhatsApp State Machine Engine (tenant-aware)
 * ============================================================
 * Implements the interactive ordering flow with Global Discovery.
 *
 * ARCHITECTURE (Phase 2 — PR 14):
 * - No module-level BUSINESS_ID/OUTLET_ID. All tenant identity comes
 *   from the `tenant` object passed to `createEngine(tenant)`.
 * - Each engine instance has its own session cache and cleanup timer.
 * - The persisted session path stays tenant-scoped:
 *   `system/botSessions/<bid>/<oid>/<safeJid>`.
 * ============================================================
 */

const { db, tenantContext, getGlobalData, getData } = require('./firebase');
...
function createEngine(tenant) {
  ...
  if (!businessId || !outletId) {
    throw new Error(
      `[Engine] createEngine requires {businessId, outletId} — got ` +
      `bid="${businessId}" oid="${outletId}" (label="${label}").`
    );
  }

  const log = (...args) => console.log(`[${label}]`, ...args);
  const logErr = (...args) => console.error(`[${label}]`, ...args);

  // Tenant-scoped helpers. `t.getUserProfile` / `t.saveUserProfile`
  // are always bound to the bot's outlet (the user's profile lives
  // at the bot's tenant, not at whatever outlet the user navigated
  // to via discovery).
  const t = tenantContext(tenant);

  // Resolve the bid for a given user. In GLOBAL discovery mode the
  // user can navigate to a different business; in that case their
  // active bid/oid is what we should read/write under. Otherwise
  // it's the bot's own tenant.
  const bidFor = (user) => user.activeBid || businessId;

  // ─── Session Cache (per-engine) ───────────────────────────
  ...
  async function persistSession(jid, session) {
    try {
      ...
      const path = `system/botSessions/${businessId}/${outletId}/${safeJid}`;
      await db.ref(path).set(session);
      setCachedSession(jid, session);
    } catch (err) {
      logErr(`[Session] Failed to persist for ${jid}:`, err.message);
    }
  }
  ...
  async function getSession(jid) {
    ...
    try {
      ...
      logErr(`[Session] Failed to load for ${jid}:`, err.message);
    }
    return null;
  }
  ...
  async function handleIncomingMessage(sock, m) {
    ...
    if (!user) {
      const profile = await t.getUserProfile(senderJid);
      user = {
        step: 'START',
        cart: [],
        current: {},
        activeOutlet: outletId === 'GLOBAL' ? null : outletId,
        activeBid: businessId,
    ...
    if (text.toLowerCase() === 'reset' || text.toLowerCase() === 'menu') {
      ...
      user.cart = [];
      user.current = {};
      if (outletId === 'GLOBAL') user.activeOutlet = null;
      ...
      await logBotAudit('BOT_RESET', { reason: text.toLowerCase() }, senderJid, tenant);
      return;
    }
    ...
    try {
      ...
      logErr(`[Bot Error] ${senderJid}:`, err);
  ...
  async function handleStart(sock, jid, user) {
    if (outletId === 'GLOBAL' && !user.activeOutlet) {
  ...
  async function showMenu(sock, jid, user) {
    ...
    const store = await getData('settings/Store', bidFor(user), oid);
    const botSettings = await getData('settings/Bot', bidFor(user), oid) || {};
    ...
    const categories = await getData('categories', bidFor(user), oid);
    if (!categories) {
      user.step = 'START';
    ...
    msg += `\n🛒 *9* View Cart\n${outletId === 'GLOBAL' ? '0️⃣ *Change Shop* 🏘️' : ''}\n_Reply with a number to browse_`;
  ...
  async function handleCategorySelection(sock, jid, user, text) {
    ...
    const dishes = await getData('dishes', bidFor(user), oid) || {};
    const inventory = await getData('inventory', bidFor(user), oid) || {};
  ...
  async function handleQuantitySelection(sock, jid, user, text) {
    ...
    user.cart.push({
      menuItemId: user.current.dish.id,
  ...
  async function handleLocationReceived(sock, jid, user, location, text) {
    ...
    const store = await getData('settings/Store', bidFor(user), oid) || {};
    const delSettings = await getData('settings/Delivery', bidFor(user), oid) || {};
  ...
  async function generateNextOrderId(bid, oid) {
    const today = new Date();
    const dateStr =
      `${today.getFullYear()}` +
      `${(today.getMonth() + 1).toString().padStart(2, "0")}` +
      `${today.getDate().toString().padStart(2, "0")}`;
    const seqPath = `businesses/${bid}/outlets/${oid}/metadata/orderSequence/${dateStr}`;
    const result = await db.ref(seqPath).transaction((current) => (current || 0) + 1);
    if (!result.committed) {
      throw new Error('Order ID transaction aborted');
    }
    const seqNum = result.snapshot.val();
    return `${dateStr}-${String(seqNum).padStart(4, "0")}`;
  }

  async function handleFinalCheckout(sock, jid, user, text) {
    const oid = user.activeOutlet;
    const bid = user.activeBid || businessId;
    ...
    if (text === "1") {
      let orderId;
      try {
        orderId = await generateNextOrderId(bid, oid);
      } catch (err) {
        logErr(`[Order] Failed to generate order id for ${jid}:`, err.message);
        await sock.sendMessage(jid, {
          text: "⚠️ We couldn't place your order right now. Please try again in a moment."
        });
        return;
      }
      ...
      for (const item of user.cart) {
        ...
        const dishRef = db.ref(`businesses/${bid}/outlets/${oid}/dishes/${item.menuItemId}`);
        try {
          const snap = await dishRef.once('value');
          if (!snap.exists()) continue;
          const dish = snap.val();
          if (dish.stock === undefined || dish.stock === null) continue;
          if (typeof dish.stock === 'boolean') {
            await dishRef.update({ stock: true, updatedAt: new Date().toISOString() });
            log('DISH_STOCK', item.name, 'flagged available after order');
          } else {
            const newStock = Math.max(0, (Number(dish.stock) || 0) - item.quantity);
            await dishRef.update({ stock: newStock, updatedAt: new Date().toISOString() });
            log('DISH_STOCK', item.name, 'decremented by', item.quantity, '→', newStock);
          }
        } catch (stockErr) {
          logErr(`[Order] Stock decrement failed for ${item.name}:`, stockErr.message);
        }
      }

      await t.saveUserProfile(jid, {
        name: user.name,
        phone: user.phone,
        address: user.address,
  ...
  return {
    ...
    // Exposed for the orchestrator and tests
    _tenant: tenant,
    log
  };
}
