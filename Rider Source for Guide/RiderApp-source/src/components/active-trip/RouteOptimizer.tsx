  ...
  const stops = useMemo(() => {
    ...
    return all.map((o) => {
      ...
      return {
        ...
        address: atOutlet ? o.outletAddress : o.address,
    ...
    <>
      <div className="flex items-center gap-2.5 rounded-[14px] bg-[#EAF2FF] border border-[#CFE0FF] px-3.5 py-2.5 mb-3.5">
        ...
        <div className="min-w-0">
          <b className="block text-[12px] text-[#1D4ED8]">Multi-Drop Route &middot; {secondary.length + 1} stops</b>
          <span className="block text-[10.5px] text-[#3B5FC4]">Nearest-neighbor sequencing available</span>
        </div>
        <button
