  ...
  return (
    <div className="rounded-[20px] bg-card border border-border/70 shadow-[var(--shadow-card)] p-3.5 mb-3">
      ...
      <div className="flex items-center gap-1.5 text-[11.5px] font-semibold text-muted-foreground mb-2.5">
        <MapPin size={12} />
        {order.distance !== undefined ? `${order.distance.toFixed(1)} km away` : ""} &middot; {order.outletAddress || order.outletName}
      </div>
      ...
      <button
        ...
        disabled={accepting}
        className="w-full rounded-[11px] bg-primary py-3 text-[12.5px] font-extrabold tracking-wide text-white shadow-[0_6px_16px_var(--primary-glow)] disabled:opacity-60 active:scale-[0.98] transition-transform"
      >
