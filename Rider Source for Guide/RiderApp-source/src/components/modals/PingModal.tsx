...
// Full-screen new-order alert with a 30s countdown (PRD §12.15). Mounted once at
// the app shell level so it fires no matter which tab the rider is looking at.
...
export function PingModal() {
  ...
  // Detect newly-appeared, in-range orders while Online with no active delivery.
  useEffect(() => {
    if (!initializedRef.current) {
      // Don't ping for orders that already existed when the app first loaded.
    ...
    const fresh = orders.filter(
      (o) =>
        ...
        !ignoredIdsRef.current.has(o.id) &&
        (o.distance === undefined || o.distance <= PROXIMITY.ACCEPT_ORDER_KM)
    );
    if (fresh.length > 0) {
      fresh.forEach((o) => seenIdsRef.current.add(o.id));
      setQueue((q) => [...q, ...fresh]);
    }
    // Keep the seen-set in sync so orders taken by others don't re-trigger.
    orders.forEach((o) => seenIdsRef.current.add(o.id));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [orders, isOnline, activeOrder]);

  // Pop the next queued ping when idle.
  useEffect(() => {
  ...
  async function handleAccept() {
    ...
    setAccepting(true);
    const payload = {
      bid: current.businessId,
      oid: current.outletId,
      orderId: current.id,
  ...
  return (
    <div className="fixed inset-0 z-[70] flex flex-col items-center justify-center px-6 py-8 bg-gradient-to-b from-[#0F1720] to-[#1a1008]">
      ...
      <p className="text-[12.5px] font-bold text-[#FF9D66] uppercase tracking-wide mt-0.5">{current.outletName}</p>
