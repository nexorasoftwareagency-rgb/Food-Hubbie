...
function OrderTaskPanel({ order }: { order: ActiveOrder }) {
  ...
  const target = atOutlet
    ? {
        ...
        address: order.outletAddress || order.outletName,
        lat: order.outletLat,
        lng: order.outletLng,
  ...
  const distanceKm = location ? getDistanceKm(location.lat, location.lng, target.lat, target.lng) : null;
  const threshold =
    step === 1 ? PROXIMITY.CONFIRM_PICKUP_KM : step === 0 ? PROXIMITY.REACHED_OUTLET_KM : order.riderAcceptanceRadius || PROXIMITY.DEFAULT_DROP_RADIUS_KM;
  const proximityOk = distanceKm === null || distanceKm <= threshold;
    ...
    try {
      if (!navigator.onLine) {
        if (step === 0) {
          ...
          setVerifyOpen(true); // checklist works fully offline-safe; confirmPickup itself gates below
        } else if (step === 2) {
          enqueueOfflineAction("UPDATE_STATUS", {
            ...
            dropLat: order.lat,
            dropLng: order.lng,
            radiusKm: order.riderAcceptanceRadius,
            customerPhone: order.customerPhone || order.phone,
            existingOtp: order.deliveryOTP || order.otp,
          });
          toast.warning("You're offline", { description: "This will sync automatically once you're back online." });
        }
        return;
      ...
      if (step === 0) {
        ...
        const { otp } = await markReachedDrop({
          bid: order.bid,
          oid: order.oid,
          orderId: order.id,
          riderUid: user!.uid,
  ...
  async function handleConfirmPayment(method: "cod" | "upi") {
    if (!user?.uid) return;
    setPayLoading(true);
    try {
...
export function ActiveTripView() {
  ...
  const primaryTarget = activeOrder.step < 2
    ? { lat: activeOrder.outletLat, lng: activeOrder.outletLng, label: activeOrder.outletName, color: "#FF5200" }
