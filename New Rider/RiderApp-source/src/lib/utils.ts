...
/** Haversine great-circle distance in km (PRD §12.16) */
...
/** Nearest-neighbor route optimization for multi-drop orders (PRD §12.17) */
...
/** Maps an order's status/timestamps onto the 4-step delivery pipeline (PRD §12.2) */
...
/** True if an order is too old to be trustworthy (PRD §12.11) */
...
/** Haptic feedback wrapper (PRD §12.19) */
...
/** Compresses/resizes an image File client-side before upload (PRD §12.7) */
export async function compressImage(file: File, targetSizeKB = 200, maxWidth = 1024): Promise<Blob> {
...
/** Fully clears service workers, caches, and local state, then force-reloads (PRD §12.13) */
