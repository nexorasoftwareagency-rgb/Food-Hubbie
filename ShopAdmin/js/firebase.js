/**
 * FOODHUBBIE SAAS | ShopAdmin Firebase Service
 * Handles Firebase initialization, database references, and multi-tenant outlet scoping.
 * Refactored from legacy flat paths (pizza/orders) to SaaS paths (businesses/{bid}/outlets/{oid}/orders).
 */
...
/**
 * MULTI-TENANT OUTLET HELPER
 * SaaS path resolution: businesses/{businessId}/outlets/{outletId}/{path}
 * Legacy paths (pizza/orders) are now fully replaced.
 */
export const Outlet = {
    // Default business — in SaaS mode this will be set during login
    get businessId() {
        return window.currentBusinessId || sessionStorage.getItem('adminBusinessId') || 'business_roshani';
    },
    get current() {
        // Fallback chain: Window -> SessionStorage -> Default
        let outlet = window.currentOutlet || sessionStorage.getItem('adminSelectedOutlet') || 'outlet_pizza';
        outlet = outlet.toLowerCase().trim();
        if (!outlet) outlet = 'outlet_pizza';
        return outlet;
    },
    ref(path) {
        ...
        if (globalPaths.includes(firstSegment)) {
            ...
            // SaaS multi-tenant path: businesses/{bid}/outlets/{oid}/{path}
            finalPath = `businesses/${this.businessId}/outlets/${this.current}/${cleanPath}`;
            console.log(`[Outlet] SaaS Path: "${path}" -> "${finalPath}"`);
        }
...
window.forceOutlet = (outletId, businessId) => {
    console.warn(`[Outlet] OVERRIDE: business=${businessId || Outlet.businessId}, outlet=${outletId}`);
    window.currentOutlet = outletId;
    sessionStorage.setItem('adminSelectedOutlet', outletId);
    if (businessId) {
        window.currentBusinessId = businessId;
        sessionStorage.setItem('adminBusinessId', businessId);
    }
