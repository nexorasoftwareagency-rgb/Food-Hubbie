...
export function updateBranding() {
    ...
    const outletId = state.currentOutlet;

    // SaaS: Derive display label from outletId
    const label = outletId.replace('outlet_', '').toUpperCase() + ' OUTLET';
    const isPizza = outletId.includes('pizza');
    const primary = isPizza ? 'var(--primary-pizza)' : 'var(--primary-cake)';

    // Apply color variables via data-outlet
    document.documentElement.setAttribute('data-outlet', isPizza ? 'pizza' : 'cake');
    ...
    if (sidebarBrand) {
        sidebarBrand.innerText = 'FOODHUBBIE';
    }

    document.title = `Foodhubbie | ${label} Dashboard`;
...
export function openOutletInNewTab() {
    const other = state.currentOutlet.includes('pizza') ? 'outlet_cake' : 'outlet_pizza';
    const url = new URL(window.location.origin + window.location.pathname);
    url.searchParams.set('outlet', other);
