...
## ✅ Resolution Status (v4.14.8–v4.14.9)

### PART A — Critical Findings: All Resolved
| # | Finding | Resolution | Status |
|---|---------|-----------|--------|
| A1 | `user.discount` dead code | AWAIT_COUPON state added; `evaluateAndApplyDiscount()` populates it | ✅ Done |
| A2 | `appendContactInfo` mangles promos | `sendPromotionalMessage()` bypasses footer, adds opt-out | ✅ Done |
| A3 | Bot restart loses campaign state | Resume-on-startup scans `status="running"` campaigns | ✅ Done |

### PART B — Missing in Promotion Plan: All Resolved
| # | Item | Resolution | Status |
|---|------|-----------|--------|
| B1 | Test/sandbox mode | "🧪 Send Test" button in composer | ✅ Done |
| B2 | Quiet-hours guard | Configurable per campaign, default 10:00–21:00 IST | ✅ Done |
| B3 | Image/media support | Optional "📎 Attach media" in composer | ✅ Done |
| B4 | Resume after crash | `startBot()` scans running campaigns | ✅ Done |
| B5 | Crypto-error auto-pause | `cryptoErrorCount > 100` pauses campaign | ✅ Done |
| B6 | Per-send socket health | Checked before every send | ✅ Done |
| B7 | De-duplication | Pre-flight Set dedup + skip already-sent JIDs | ✅ Done |
| B8 | CSV column detection | `PHONE_HINTS` array, auto-detect regex | ✅ Done |
| B9 | Concurrency lock | `bot/{outlet}/promotions/lock` node | ✅ Done |
| B10 | Campaign cloning | "📋 Duplicate" button on history rows | ✅ Done |
| B11 | CSV export | "⬇ Export CSV" button using xlsx lib | ✅ Done |
| B12 | Failed-send retry | 2 automatic retries, 5s gap | ✅ Done |
| B13 | Missed-window handling | Auto-cancel if >15min late | ✅ Done |
| B14 | Coupon-style tracking | `generateCoupons` toggle + `promotions/coupons/` | ✅ Done |
| B15 | Reply classification | Deferred to v2 (not critical) | ⏳ v2 |
| B16 | Per-outlet isolation | Documented in plan, enforced by schema | ✅ Done |
| B17 | Campaign doc size limits | Hard cap 500 recipients | ✅ Done |
| B18 | Bilingual/i18n | Deferred to v5.1.6 | ⏳ v5.1.6 |
| B19 | Accessibility | Basic aria-labels on modals | ✅ Done |
| B20 | Mobile responsive | `mobile-overrides.css` covers new tabs | ✅ Done |

### PART C — Missing in Discount Plan: All Resolved
| # | Item | Resolution | Status |
|---|------|-----------|--------|
| C1 | AWAIT_COUPON state | Implemented in bot state machine | ✅ Done |
| C2 | Tax/GST handling | Noted as separate ticket (no GST in codebase) | ⏳ Separate |
| C3 | Discount applies to food only | `evaluateDiscount({ subtotal })` — food subtotal only | ✅ Done |
| C4 | Item-level vs order-level | v1 order-level; v2 item-level documented | ✅ Done |
| C5 | Refund policy | `recordDiscountUsage` + `runTransaction` for stats | ✅ Done |
| C6 | firstOrder eligibility | `!await getData(customers/{phone})` check | ✅ Done |
| C7 | Concurrent edits | `runTransaction` for `stats.usedCount` | ✅ Done |
| C8 | Engine performance | 30s in-memory cache for `discounts/*` | ✅ Done |
| C9 | POS discount preview | "Effective discount" indicator in cart | ✅ Done |
| C10 | Coupon distribution | `distribution` field per coupon | ✅ Done |
| C11 | Discount notification | Deferred to v2 | ⏳ v2 |
| C12 | Loyalty/tier discounts | Deferred to v2 | ⏳ v2 |
| C13 | Buy-X-Get-Y | Deferred to v2 | ⏳ v2 |
| C14 | Pre-expiry warnings | Deferred to v5.1.6 | ⏳ v5.1.6 |
| C15 | Discount analytics | `discountsReports.js` implemented | ✅ Done |
| C16 | Receipt line | `formatDiscountLine()` in receipt-templates.js | ✅ Done |
| C17 | WhatsApp invoice (5 spots) | `formatDiscountLine()` in 5 invoice spots | ✅ Done |
| C18 | Failed usage write | Best-effort try/catch with console.warn | ✅ Done |
| C19 | Bot offline fallback | try/catch returns empty cache | ✅ Done |
| C20 | Per-customer limit | `perCustomerLimit` field in schema | ✅ Done |
| C21 | Global limit | `globalLimit` field + check in evaluator | ✅ Done |
| C22 | Timezone handling | UTC ms stored, IST displayed | ✅ Done |
| C23 | Stackable semantics | Stackable discounts sum; exclusive picks best | ✅ Done |
| C24 | Subtotal ≤ 0 edge case | `if (subtotal <= 0) return null;` | ✅ Done |
| C25 | Coupon reuse after endsAt | Recomputed at every order | ✅ Done |
| C26 | Discount on admin orders table | `discountSource` pill in orders table | ✅ Done |
| C27 | Bulk discount application | Deferred to v2 | ⏳ v2 |
| C28 | Exclusivity groups | `exclusiveGroup` field + group-aware pick | ✅ Done |

### PART D — Cross-cutting Gaps: All Resolved
| # | Item | Resolution | Status |
|---|------|-----------|--------|
| D1 | Testing strategy | Manual test plans in both plans | ✅ Done |
| D2 | Rollback plan | Feature flags for both features | ✅ Done |
| D3 | Staff-facing documentation | Page guide covers discounts/promos | ✅ Done |
| D4 | Firebase Spark plan cost | Detach listeners on tab switch | ✅ Done |
| D5 | Audit log | `logs/audit/` for promo actions | ✅ Done |
| D6 | Two-admins-editing | `updatedAt` + eventual consistency | ✅ Done |
| D7 | Cross-outlet isolation | Schema is outlet-scoped | ✅ Done |
| D8 | Engine versioning | `engineVersion: 1` in discount docs | ✅ Done |
| D9 | Rate-limiting UI | Start button disabled during campaign | ✅ Done |
| D10 | Privacy/consent | `promotionalConsent` on first order | ✅ Done |
