...
export async function loadLostSales() {
    ...
    try {
        ...
        if (!data) {
            ...
            <strong>${t('lostSales.empty', 'No lost sales found!')}</strong><br>${t('lostSales.emptySub', 'All your customers are reaching the finish line.')}
        </td></tr>`;
        ...
        sorted.forEach(([id, record]) => {
            ...
            const itemsStr = items.map(i => `${i.name || i.item} (${i.size || t('lostSales.fallback.na', 'N/A')})`).join(', ');
            const ts = formatDate(record.cancelledAt);
            const source = record.sourceStep || t('lostSales.fallback.checkout', 'Checkout');

            const phone = record.phone || t('lostSales.fallback.na', 'N/A');
            const whatsappLink = `https://wa.me/91${phone.replace(/\D/g, '').slice(-10)}`;

            const tr = document.createElement('tr');
            tr.className = 'premium-row-v4';
            tr.innerHTML = `
            <td data-label="${t('lostSales.col.date', 'Date')}" class="p-l-25">
                <div class="identity-info-v4">
                    <span class="name">${escapeHtml(ts)}</span>
                    <span class="sub">ID: ...${escapeHtml(id.slice(-6))}</span>
                </div>
            </td>
            <td data-label="${t('lostSales.col.customer', 'Customer')}">
                <div class="identity-info-v4">
                    <span class="name">${escapeHtml(record.customerName || t('lostSales.fallback.guest', 'Guest'))}</span>
            ...
            <td data-label="${t('lostSales.col.step', 'Step')}">
            ...
            <td data-label="${t('lostSales.col.items', 'Items')}" style="max-width:250px;">
                <div class="identity-info-v4">
                    <span class="sub text-truncate-2" title="${escapeHtml(itemsStr)}">${escapeHtml(itemsStr)}</span>
                </div>
            </td>
            <td data-label="${t('lostSales.col.value', 'Value')}" class="p-r-25 text-right">
        ...
        tbody.innerHTML = `<tr><td colspan="5" style="text-align:center; padding:40px; color:red;">${t('lostSales.loadError', 'Error loading data. Check console.')}</td></tr>`;
    }
}
...
export async function clearLostSales() {
    ...
    try {
        ...
        showToast(t('lostSales.clearSuccess', 'Logs cleared successfully'), 'success');
        loadLostSales();
    } catch (e) {
        console.error('Clear Logs Error:', e);
        showToast(t('lostSales.clearFailed', 'Failed to clear logs'), 'error');
    }
}
