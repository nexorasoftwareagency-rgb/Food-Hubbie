...
function renderCustomersPage(page) {
    ...
    table.innerHTML = pageItems.map(item => `
        <tr class="premium-row-v4">
            <td data-label="${t('customers.col.customer', 'Customer')}">
                <div class="identity-chip-v4">
                    ...
                    <div class="identity-info-v4">
                        ...
                        <span class="sub">${t('customers.joined', 'Joined: {date}').replace('{date}', item.joined)}</span>
                    </div>
                </div>
            </td>
            <td data-label="${t('customers.col.whatsapp', 'WhatsApp')}">
                <div class="identity-info-v4">
            ...
            <td data-label="${t('customers.col.address', 'Address')}">
                <div class="identity-info-v4">
                    <span class="sub" title="${escapeHtml(item.addressFull)}">${escapeHtml(item.address)}</span>
                    ${item.locationLink ? `<a href="${escapeHtml(item.locationLink)}" target="_blank" rel="noopener noreferrer" class="link-premium fs-10 font-bold"> ${t('customers.viewMap', 'VIEW MAP')}</a>` : ""}
                </div>
            </td>
            <td data-label="${t('customers.col.orders', 'Orders')}">
                <div class="flex-col">
                    <span class="font-bold color-primary">${item.orderCount}</span>
                    <span class="text-muted-small">${t('customers.purchases', 'Purchases')}</span>
                </div>
            </td>
            <td data-label="${t('customers.col.value', 'Value')}" class="text-right">
                <div class="flex-col pr-15">
                    <span class="font-bold fs-15">₹${item.ltv.toLocaleString()}</span>
                    <span class="text-muted-small">${t('customers.ltv', 'LTV')}</span>
                </div>
            </td>
        </tr>
    `).join('');
...
export async function loadCustomers() {
    ...
    custSnap.forEach(child => {
        ...
        _customersData.push({
            name: c.name || t('customers.fallback.anonymous', 'Anonymous'),
            joined: c.registeredAt ? new Date(c.registeredAt).toLocaleDateString() : t('customers.fallback.na', 'N/A'),
            displayPhone: phone,
            phoneClean: phone.replace(/\D/g, "").slice(-10),
            address: c.address ? (c.address.length > 30 ? c.address.substring(0, 30) + "..." : c.address) : t('customers.fallback.counterSale', 'Counter Sale / Guest'),
